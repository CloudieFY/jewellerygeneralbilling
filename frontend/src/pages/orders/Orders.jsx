...
const Orders = () => {
  ...
  const orderMessage = (order) => [
    ...
    `Fixed Rate: Rs ${Number(order.fixedRate).toLocaleString("en-IN")}`,
    `Advance Paid: Rs ${Number(order.advancePaid).toLocaleString("en-IN")}`,
    `Balance: Rs ${(Number(order.fixedRate) - Number(order.advancePaid)).toLocaleString("en-IN")}`,
    `Due Date: ${new Date(order.dueDate).toLocaleDateString("en-IN")}`,
    order.note ? `Note: ${order.note}` : "", "Please confirm this order and design.",
  ].filter(Boolean).join("\n");
      ...
      <form ref={formRef} onSubmit={submit} className="premium-card overflow-hidden">
        ...
        <div className="grid gap-7 p-5 sm:p-7 lg:grid-cols-2">
          ...
          <section className="space-y-4">
            ...
            <div className="grid gap-4 sm:grid-cols-2"><label className="text-xs font-black uppercase tracking-wider text-slate-500">Fixed rate (Rs) *<input type="number" min="0" className="input-field mt-2 normal-case" value={form.fixedRate} onChange={(e) => setField("fixedRate", e.target.value)} required /></label><label className="text-xs font-black uppercase tracking-wider text-slate-500">Advance paid (Rs)<input type="number" min="0" className="input-field mt-2 normal-case" value={form.advancePaid} onChange={(e) => setField("advancePaid", e.target.value)} /></label></div>
        ...
        <div className="flex flex-col gap-3 border-t border-amber-100 bg-amber-50/40 p-5 sm:flex-row sm:items-center sm:justify-between sm:p-7"><p className="text-sm font-bold text-slate-600">Balance: Rs {Math.max(0, Number(form.fixedRate || 0) - Number(form.advancePaid || 0)).toLocaleString("en-IN")}</p><button disabled={saving} className="btn-primary inline-flex items-center justify-center gap-2 disabled:opacity-50"><CheckCircle2 size={19}/>{saving ? "Saving order..." : "Save Order"}</button></div>
      </form>
