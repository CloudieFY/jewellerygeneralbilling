...
export const calculateLine = (item, product, _rateType, gstEnabled = true) => {
  ...
  const netWeight = toNumber(item?.netWeight ?? product?.netWeight) * quantity;
  const grossWeight = toNumber(item?.grossWeight ?? product?.grossWeight) * quantity;
  const stoneWeight = toNumber(item?.stoneWeight ?? product?.stoneWeight) * quantity;
  ...
  const stoneValue = toNumber(item?.stoneValue ?? product?.stoneValue) * quantity;
  const discount = toNumber(item?.discount);
  const baseAmount = Math.max(metalValue + wastageAmount + makingChargeAmount + stoneValue - discount, 0);
  const gstRate = gstEnabled ? toNumber(item?.gstRate ?? product?.gstRate, 3) : 0;
  const gstAmount = baseAmount * gstRate / 100;
  return { quantity, netWeight, grossWeight, stoneWeight, rate, metalValue, wastagePercent, wastageAmount, makingChargeType, makingCharge, makingChargeAmount, stoneValue, discount, baseAmount, gstRate, gstAmount, lineTotal: baseAmount + gstAmount, sqFt: 0 };
};
